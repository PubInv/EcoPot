...
module conicalLidIvan(inner_rad,A,V_pot){
    ...
    difference(){
        ...
        translate([0,0,lidPositionZ(A,V_pot)+rim_bead_radius/2]){
        rotate ([180,0,0]){    
        lidInterface(inner_rad,inner_rad+wall_thickness,
                    rim_bead_radius);}
            }          
...
module handle(A,V,ptype,radius){
    ...
    else if (ptype=="dimplesWithHandles"){
        difference() {
            pothandleshell(radius);
            union(){
            dimples(A,V_pot);
...
//concept Gianluca
module dimplesWithHandles(A,V) {
    radius_mm = radius(A,V);
    radius_ball=radius_mm/4;
 difference  () {
    ...
    roundBottomPot(A,V_pot);
        for (phi = [0 :20: 70]) {
            ...
            for (theta = [0 :step: 360]) {
                rotate ([0,phi,theta])
                translate([radius_mm+radius_ball+(wall_thickness/3),0,0])
                sphere( radius_ball);
                }
            }
        };
