//VERSION = 1.01;
excess_lip_scale_factor = 1.3; //increase the size of h
PI = 3.141592;
...
HEIGHT_STRETCH = 0.7; 
LID_FACTOR = 4; 
POT_VERTICAL_OFFSET = 48; 
Amp = 20; 
N = 4;  

WALL = RIM_RADIUS / 25;  
OUTER_R = RIM_RADIUS + WALL;
H = BASE_HEIGHT; 
rim_bead_radius = RIM_RADIUS / 25;
lid_thickness = WALL;
...
// Handle Settings
lid_handle_radius = RIM_RADIUS * 0.6;          
lid_handle_thickness = RIM_RADIUS / 6;       
pot_handle_radius = RIM_RADIUS / 2.0; 
pot_handle_thickness = RIM_RADIUS / 6;
handle_position_z = -10; 

real_z_at_rim = (BASE_HEIGHT == RIM_RADIUS) ? BASE_HEIGHT : sqrt(max(0, pow(H, 2) - pow(RIM_RADIUS, 2)));
pot_total_height = real_z_at_rim * HEIGHT_STRETCH;
conical_lid_scale_factor = 0.7;
lid_height = (H * 0.45) * HEIGHT_STRETCH * LID_FACTOR; 

// Grid setup for polyhedrons
grid_size = 100; 
dx = (2 * OUTER_R)/grid_size;
dy = (2 * OUTER_R)/grid_size;
...
module conical_part(radius, height){
    translate([0,0, pot_total_height + lid_thickness*0.55]){
...
module conicalLidIvan(inner_rad){
    outer_rad = inner_rad + WALL;
    translate([0,0, -rim_bead_radius/2]){
        union(){
            difference(){
                conical_part(outer_rad, lid_height);
                translate([0,0, WALL])
                conical_part(inner_rad, lid_height - WALL + 0.001);
            }
            translate([0,0, pot_total_height])
            rotate([180,0,0]);
        }
    }
}
...
module lidhandleshell (){
    translate([0,0, lid_height + (lid_handle_thickness/2)]){
...
module pothandleshell(y_offset) {
  translate([0, y_offset, handle_position_z])
  rotate_extrude(angle = 360) {
...
// TODO:
// Change this code so that changing t effects only the outside of the pot (done)
...
// Polyhedron Topology
faces_a = [[3,2,5],[4,0,1],[0,2,1],[2,3,1],[1,3,4],[3,5,4],[5,2,4],[2,0,4]];
faces_b = [[4,5,1],[2,0,3],[5,4,2],[2,3,5],[4,1,0],[0,2,4],[1,5,3],[3,0,1]];
...
module pot_handle(side_sign){
    difference(){
        pothandleshell(side_sign * RIM_RADIUS);
        // Clipping offset to prevent Z-fighting
        cylinder(r = RIM_RADIUS + 0.8, h = RIM_RADIUS*2, center= true);
    }
}
...
module renderPotType(){
    translate([0,0, POT_VERTICAL_OFFSET]){
        union(){
            wavy_pot();
...
echo("Rim Bead Radius:", rim_bead_radius);
